...
app = Flask(__name__) 
app.secret_key = "SECRET KEY" # hange later
app.permanent_session_lifetime = timedelta(days=5) #how long you stay logged in automatically
#is this cookies?
...
def index():
    ...
    if token == None and not "user" in session: #or if user not already logged in
        ...
        info = authtoken.get_userinfo()
        if User.query.filter_by(uid=info["uid"]).first(): #if user exists in db
            #find user, add new authcode to their db *********
            session.permanent = True
            session["user"]=info["name"]
            return render_template('index.html', data=data) # will this data load the user's data upon login?
        else: #if user does not exist, add them create user
            username = User(username=info["name"], uid=info["uid"], image_file=info["image"], authcode=token) #this can go into get_userinfo now
...
def login():
    ...
    data = json.load(open(json_url))
    token = request.args.get('code')
    if token and not "user" in session:
        info = authtoken.get_userinfo()
        username = User(username=info["name"], uid=info["uid"], image_file=info["image"], authcode=token)
        db.session.add(username)
        db.session.commit()
        session.permanent = True
        session["user"]=info["name"]
        return redirect(url_for('index'))
    elif token and "user" in session:
        info = authtoken.get_userinfo()
        #save code to user, find in db
        return redirect(url_for('index'))

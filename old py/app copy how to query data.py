...
def users():
   users = User.query.all() #this works
   fusernames = User.query.order_by(User.username).all() #this also works
   #answers = Answer.query.filter_by(day = fusernames)
   answers = User.query.filter_by(email = "james")
   return answers

#user = User.query.filter_by(username = "YAY IT WORKS!")
#print(user)
...
#ways to query data
#stmt = select(User).where(User.username == "YAY IT WORKS") # pure sql, from sqlalchemy import select # see https://docs.sqlalchemy.org/en/20/tutorial/data_select.html#tutorial-selecting-data
userall = db.session.scalars(db.select(User)).all() # or User.query.all()
user1 = db.get_or_404(User, 1) #gets all user info by id
user2 = user = db.session.execute(db.select(User).filter_by(email="itworks@gmail.com")).scalars().first() #this also works
user3= db.session.scalars(db.select(User).where(User.email == "itworks@gmail.com")).first()
user4= User.query.filter_by(email="itworks@gmail.com").first()
print(user4.email)
print("i reach here2")


if User.query.filter_by(email="itw@gmail.com").first():
    print("yes")
else:
    print("no")
#how to query data in database
# user = db.session.execute(db.select(User).filter_by(id=1)).scalars().first()
# WHERE id can be replaced with username, or any sort of dataset in that table
# NOW to get a list, i would simply get rid of the .first() to search for a group of datasets where the filter = something
#to see all available data, i can use User.query.all() OR for a specific thing User.query.order_by(User.username).all()
#
#to select a particular text of data from the data set. i would simply use the resultant and target. so user.email returns email, #or user.username returns user name!
# into db with terminal
#
